"""Tests for backpack.paths - filesystem location helpers."""
...
class TestApplogs:
    def test_windows_uses_localappdata(self) -> None:
        with patch("backpack.paths.sys.platform", "win32"), patch.dict(
            "os.environ", {"LOCALAPPDATA": "C:/Local"}, clear=True
        ), patch("backpack.paths.Path.home", return_value=Path("C:/home")):
    ...
    def test_windows_falls_back_to_home(self) -> None:
        ...
        ), patch("backpack.paths.Path.home", return_value=home):
            assert applogs() == home / "AppData/Local" / APP_NAME / "Logs"

    def test_macos_uses_library_logs(self) -> None:
        home = Path("/Users/tester")
        with patch("backpack.paths.sys.platform", "darwin"), patch(
            "backpack.paths.Path.home", return_value=home
        ):
            assert applogs() == home / "Library/Logs" / APP_NAME

    def test_linux_uses_xdg_state_home(self) -> None:
        with patch("backpack.paths.sys.platform", "linux"), patch.dict(
            "os.environ", {"XDG_STATE_HOME": "/xdg/state"}, clear=True
        ), patch("backpack.paths.Path.home", return_value=Path("/home/x")):
    ...
    def test_does_not_create_directory(self) -> None:
        with patch("backpack.paths.sys.platform", "darwin"), patch(
            "backpack.paths.Path.home", return_value=Path("/Users/tester")
        ):
