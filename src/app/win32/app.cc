	...
	url(std::move(url))
{
	...
	webview_.on_create = [this, hwnd](HRESULT hr) {
		if (FAILED(hr)) {
		...
		webview_.navigate(this->url);
	};
	webview_.on_msg = [this](std::string json) -> HRESULT {
		event_q.push(std::move(json));
		return S_OK;
	};
	webview_.on_load = [this] (bool) {
		event_q.push("{ \"name\": \"load\", \"args\": [] }");
	};
	webview_.create(hwnd, L"", assets);
}

void app_t::quit(void) noexcept
try {
	if (HWND hwnd = window_.hwnd())
		call_later(hwnd, [this] { webview_.close(); });
} catch (...) {
	/* the post failed (window gone); teardown is already under way */
}
