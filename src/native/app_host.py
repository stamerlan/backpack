...
class Js:
    ...
    event loop is used. shutdown() stops the thread and cancels the in-flight
    call and anything still queued so awaiting callers fail fast.
    """

    def __init__(self, view: WebView) -> None:
        self._view = view
        self._wq: queue.Queue[JsCall | None] = queue.Queue()
        self._lock = threading.Lock()
        self._running = True
        self._current: JsCall | None = None
    ...
    def shutdown(self) -> None:
        ...
        The in-flight call may never settle once the window is gone, so it is
        cancelled here instead of waiting on the web view.
        """
        with self._lock:
            if not self._running:
                return
    ...
    def _run(self) -> None:
        ...
        while True:
            ...
            with self._lock:
                if not self._running:
                    # taken off the queue just before shutdown drained it
                    job.fut.cancel()
                    break
                self._current = job
...
class Events:
    ...
    the GUI thread for window load/close) and hands it to a waiting
    get_event(), or queues it until one asks; it returns False once shut down
    so the caller can react. get_event() returns a concurrent future for the
    next event that Core drives on its loop. A lock guards the queue and the
    single pending waiter, so no event loop is involved. shutdown() cancels
    the waiter and drops the queue so Core stops waiting.
    """

    def __init__(self) -> None:
        self._lock = threading.Lock()
        self._pending: deque[app_host.Event] = deque()
        self._waiter: Future[app_host.Event] | None = None
        self._running = True
    ...
    def post(self, name: str, args: tuple[Any, ...]) -> bool:
        ...
        with self._lock:
            if not self._running:
                return False
            event = app_host.Event(name, args)
    ...
    def get_event(self) -> "Future[app_host.Event]":
        ...
        with self._lock:
            fut: Future[app_host.Event] = Future()
            if not self._running:
                fut.cancel()
            elif self._pending:
...
class PyWebViewAppHost:
    def __init__(
        ...
        *,
        ...
        # The py2js and js2py bridges are host-owned and reached by Core
        # through exec_script and get_event; both settle work as concurrent
        # futures, so the host keeps no event loop. window load/close are
        # posted as events too, so Core drives the whole lifecycle off one
        # stream.
        self._events = Events()
    ...
    def start(self) -> None:
        ...
        Core runs its asyncio loop on its own thread; here webview.start just
        blocks running the platform GUI loop. quit() destroys the window to
        return from here, so the caller can join the core thread.
        """
        ...
        try:
            ...
            # core thread returns: fail in-flight exec_script futures and end
    ...
    def quit(self) -> None:
        ...
        Stops the py2js and js2py bridges, failing in-flight work fast
        (cancels exec_script futures and the pending get_event wait), then
        destroys the window so start() returns on the main thread.
        """
        self._js.shutdown()
        self._events.shutdown()
    ...
    def exec_script(self, func: str, args: tuple[Any, ...]) -> Future[Any]:
        ...
        return self._js.exec_script(func, args)
    ...
    def show_open_dialog(
        ...
        *,
        ...
        that settles the concurrent future directly. Filters are joined into
        the "name (spec)" strings pywebview expects, which only accepts
        letters, digits and spaces in the name.
        """
    ...
    def show_save_dialog(
        ...
        directly. Filters are joined as in show_open_dialog.
        """
        file_types = tuple(f"{name} ({spec})" for name, spec in filters)
        fut: Future[Any] = Future()
