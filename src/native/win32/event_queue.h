...
public:
	...
	/* Fire the pending callback with an empty event and drop any queued
	 * events. Further push() and set_cb() calls return false.
	 *
	 * Thread safe. The callback fires on the calling thread.
	 */
	void abort(void);
