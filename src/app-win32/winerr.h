...
public:
	template <class... Args>
	explicit winerr(DWORD code, std::format_string<Args...> fmt = "", Args &&...args)
		: std::runtime_error(fmt_errmsg(code, fmt, std::forward<Args>(args)...)), code_(code)
	{
	}
...
private:
	template <class... Args>
	static std::string fmt_errmsg(DWORD code, std::format_string<Args...> fmt, Args &&...args)
	{
